...
const sclassCreate = async (req, res) => {
    try {
        ...
        if (existingSclassByName) {
            res.send({ message: 'Désolé, ce nom de classe existe déjà' });
        } else {
...
const sclassList = async (req, res) => {
    try {
        ...
        if (sclasses.length > 0) {
            ...
            res.send({ message: "Aucune classe trouvée" });
        }
    } catch (err) {
        res.status(500).json(err);
    }
...
const deleteSclass = async (req, res) => {
    try {
        ...
        if (!deletedClass) {
            return res.send({ message: "Classe non trouvée" });
        }
...
const deleteSclasses = async (req, res) => {
    try {
        ...
        if (deletedClasses.deletedCount === 0) {
            return res.send({ message: "Aucune classe trouvée à supprimer" });
        }
